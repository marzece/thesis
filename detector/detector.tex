...
SNO+ has 9385 inward looking PMTs all mounted on a geodesic sphere referred to as
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\textwidth]{sno_casette}
    \caption[PMT Casette] {Schematic diagram of the SNO/SNO+ PMT and
    PMT housing. Dimensions in cm. Figure from~\cite{sno_detector_paper}.}
\label{fig:triggeff}
\end{figure}

The inward looking PMTs are all housed within a plastic cassette.
Each PMT is also surrounded by an array of reflective
...
are worth highlighting, as they motivate several of the upgrades from
SNO to SNO+.
sections.%\ref?

The first of these is the unstable analog trigger baseline.
XXXTODO\@: Discuss here issues such as dropout, baseline shifts,
and orphans.
...
\begin{figure}[htbp]
    ...
    \caption[Dropout Fit Example]{Diagram of the SNO+ DAQ\@.
    Figure from~\cite{richie_thesis}.}
...
\begin{figure}[htbp]
    ...
    individaul Gaussians [black dashed line] that make up the best fit.}
...
\begin{figure}[htbp]
    ...
    and simulated trigger efficiency for run 107640. Plot from~\cite{tanner_triggeff}.}
\label{fig:triggeff}
\end{figure}
...
For each run a similar dropout measurement is performed, the resulting dropout rate
is later used in simulation as part of the DAQ simulation.
As shown in Fig~\ref{fig:trigegff}, the trigger simulation, using the \textit{in-situ}
measurement of dropout, agrees well with the measured trigger efficiency.
...
this decay chain is referred to as BiPo214. %XXXTODO This chain is wrong, look it up idiot
...
More information about the laserball can be found in Ref~\cite{sno_laserball}.
