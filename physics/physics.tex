...
At higher energies neutrinos interact through a quasi-elastic
scattering, resonant pion-production, and deep in-elastic scattering~\cite{XXX}.
Those processes
...
current (CC) elastic scattering interaction~\ref{fig:ES_CC} and the
neutral current (NC) elastic scattering interaction~\ref{fig:ES_NC}.
...
\begin{equation}
    T_{e}^{\max}(E_\nu) = \frac{2E_{\nu}^{2}}{m_{\mathrm{e}}+ 2E_{\nu}^{2} }\text{.}
\end{equation}
These results are derived from first princibles in~\cite{XXX}.
...
is very useufl for neutrino experiments; thee observed electron direction
...
A discussion of the equal momentum assumption is available in Ref~\cite{neutrino_osc_subtleties}.
...
parameters and mass-squared splitting have been measured experimentally.
The parameters are not predicted from the standard model, so are theoretically
uncsontrained. %TODOXXX add a bunch of expermental citations?
The measurements of the mass-squared splittings and
other parameters are discussed in Sec.~\ref{sec:experiments}.
The absolute masses of each neutrino state are not yet known.
...
the case where $\nu_{3}$ is the least massive neutrino is knwn as the
 ...
 the normal hierarchy over the inverted hierarchy at \urltilde$2.4\sigma$ statistical
...
shown in $XXX$, and a flavor dependent charged current interaction, shown in
XXX, with the leptons around the neutrino.
