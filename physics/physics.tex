...
preserve a local $SU{(3)}_C \timesSU{(2)}_L \times U{(1)}_Y$ symmetry, where
$C$, $L$, and $Y$ respectively indicate color, left-hand chirality and weak hyperchange.
These symmetry groups specify the number of bosons that mediate each\ldots
...
magnetic moment so it can interact electro-magnetically, but these 
...
all weak interactions conserve both the total lepton number of a system, but als
