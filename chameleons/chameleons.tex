...
At the minimum value for $\Delta m^{2}_{21}$ and $\theta_{12}$ a search
is done in the $A_{e}$ and $A_{\mu}$ space to evaluate what
sort of preference exists for non-zero value.
Again, it would be preferable to perform this likelihood search without
constraining $\Delta m^{2}_{21}$ and $\theta_{12}$, however.
