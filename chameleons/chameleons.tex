...
Ultra-high energy neutrinos from the GZK production process~\citep{gzk} would be a useful
...
for $\Delta m^{2}_{21}$.
This motivates considerations of what physics might affect the solar survival
...
results.
%Some of the existing ideas for non-standard interactions in the Sun are discussed in Sec.~\ref{sec:nsi}
...
\chapter{Measurements of \textbf{$\Delta m^{2}_{21}$}}
...
In principle this means for solar neutrino experiments to be sensitive
to $\Delta m^{2}_{21}$, a good measurement of neutrino oscillation probability
in the transition region is all that is required.
However, the Sun does not provide an optimal source of neutrinos at energies
...
In principle, the transition region could be probed as well by a precise
...
A measured deviation of these survival probabilities from the expected
...
amount of time. The method that was used for nearly all the results here was
...
this section.
%This sampling method guarantees that no energy is so poorly sampled 
...
%This results in a simulation of the day-night effect for neutrino oscillation.
...
This problem was somewhat exacerbated by the distributions of some
...
\section{Mass State Simulation} %TODO better subsection titles
For the mass state simulation instead of simulating individiual neutrino
states, only the eigen-states are simulated.
For each production radius and production energy within the sun
...
energies, would not oscillate quickly enough to have an average
...
Each experiment provides its own measurement of detector
related systematic uncertainties, these uncertainties are all treated as independent,
except for the four Super-K datasets.
...
experiment's measured value with uncertainty to extract a likelihood.
...
The fit allowed all values to vary except $\theta_{13}$, which does not strongly affect
...
The reported error on at least some fit results is unbelievably
small.
...
These results show that the vacuum-enhanced mixing model
...
%This corresponds to p-value of approximately $15\%$.
...
I therefore do not consider this to be strong evidence in favor of the vacuum-enhanced
...
So even with good sensitivity at higher energies this model may not be able
to be uniquely identified using standard solar neutrino measurements.

The results found here were produced entirely using adiabatic neutrino mass state simulation,
