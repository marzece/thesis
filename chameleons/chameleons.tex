...
\section{Measurements of $\Delta m^{2}_{21}$}
...
for SuperK and Borexino the event rates are calculated in each energy bin.
...
For SuperK and Borexino the electron energy spectrum is convolved with a
...
with the exception of the four SuperK datasets.
 Each SuperK dataset has it's own systematic uncertainty associated with,
...
\end{equation}
A procedure recommended in Refs~\citep{sno_leta} and~\citep{sno_combinedt} is followed.
